...
namespace LD {
	...
	struct TVec2
	{
		...
		inline bool operator==(const TVec2& other) const { return x == other.x && y == other.y; }

		inline T LengthSquared() const { return x * x + y * y; }
		inline T Length() const { return static_cast<T>(LD_MATH_SQRT(LengthSquared())); }
		inline TVec2<T> Normalized() const { return *this / Length(); }
		...
		#define LD_VEC2_SWIZZLE(RET_VEC, SWIZZLE, ...) \
			RET_VEC<T> inline TVec2<T>::SWIZZLE() const \
			{ \
