...
namespace LD {
	...
	#define LD_MAT4_SCALAR(OP) \
		...
		inline operator OP(const TMat4<T>& m, T s) \
		{ \
	...
	struct TMat4
	{
		...
		TVec4<T> Data[4];
		inline T* GetData() { return (T*)Data->Data; }
		inline const T* GetData() const { return (T*)Data->Data; }

		inline TVec4<T>& operator[](int i) { return Data[i]; }
		inline const TVec4<T>& operator[](int i) const { return Data[i]; }
